...
async function handleExport(guildId, week, token, client) {
    await client.editOriginalInteraction(token, {
        content: "exporting now...",
        flags: 64
    });
    const league = await view_1.discordLeagueView.createView(guildId);
    if (!league) {
        await client.editOriginalInteraction(token, {
    ...
    try {
        const exporter = await (0, ea_client_1.exporterForLeague)(Number(league.leagueId), ea_client_1.ExportContext.MANUAL);
        if (week === 100) {
    ...
    async handleCommand(command, client, db, ctx) {
        ...
        const week = (() => {
            if (subCommand === "week") {
                ...
                const week = Number(exportCommand.options[0].value);
        ...
        (0, discord_utils_1.respond)(ctx, (0, discord_utils_1.deferMessageInvisible)());
