...
exports.createTwitchClient = exports.getSecret = void 0;
function getSecret() {
...
const TwitchClient = () => {
    ...
    const twitchRequester = async (fetcher) => {
        if (!token) {
            await refreshToken();
        }
    ...
    return {
        async retrieveBroadcasterInformation(twitchUrl) {
            const login = twitchUrl.substring(twitchUrl.lastIndexOf('/') + 1);
            const res = await twitchRequester(async (token) => await fetch(`https://api.twitch.tv/helix/users?login=${login}`, {
                method: "GET",
                headers: {
        ...
        async retrieveChannelInformation(broadcasterUserId) {
            const res = await twitchRequester(async (token) => await fetch(`https://api.twitch.tv/helix/channels?broadcaster_id=${broadcasterUserId}`, {
                method: "GET",
                headers: {
                    "Authorization": `Bearer ${token}`,
                    "Client-Id": `${process.env.TWITCH_CLIENT_ID}`
        ...
        async subscribeBroadcasterStreamOnline(broadcasterUserId) {
            const res = await twitchRequester(async (token) => await fetch(`https://api.twitch.tv/helix/eventsub/subscriptions`, {
                method: "POST",
                headers: {
                    "Authorization": `Bearer ${token}`,
                    "Client-Id": `${process.env.TWITCH_CLIENT_ID}`,
        ...
        async deleteSubscription(subscriptionId) {
            const res = await twitchRequester(async (token) => await fetch(`https://api.twitch.tv/helix/eventsub/subscriptions?id=${subscriptionId}`, {
                method: "DELETE",
                headers: {
                    "Authorization": `Bearer ${token}`,
                    "Client-Id": `${process.env.TWITCH_CLIENT_ID}`
...
const MockTwitchClient = {
    retrieveBroadcasterInformation: async (twitchUrl) => {
        return {
            "data": [
                {
                    "id": "141981764",
    ...
    retrieveChannelInformation: async (broadcasterUserId) => {
        return {
            "data": [
                {
    ...
    subscribeBroadcasterStreamOnline: async (broadcasterUserId) => {
        return {
            "data": [
                {
    ...
    deleteSubscription: async (sub) => { console.log(`${sub} is deleted`); }
};
...
exports.createTwitchClient = createTwitchClient;
//# sourceMappingURL=twitch_client.js.map