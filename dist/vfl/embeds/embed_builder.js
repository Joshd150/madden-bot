...
exports.VFLEmbedBuilder = exports.VFLColor = void 0;
/**
...
var VFLColor;
(function (VFLColor) {
    VFLColor[VFLColor["PRIMARY"] = 2067276] = "PRIMARY";
    VFLColor[VFLColor["SUCCESS"] = 5763719] = "SUCCESS";
    VFLColor[VFLColor["WARNING"] = 16705372] = "WARNING";
    VFLColor[VFLColor["ERROR"] = 15548997] = "ERROR";
    VFLColor[VFLColor["INFO"] = 5793266] = "INFO";
    VFLColor[VFLColor["SECONDARY"] = 10070709] = "SECONDARY";
    VFLColor[VFLColor["TRADE"] = 16739125] = "TRADE";
    VFLColor[VFLColor["GAME"] = 4286945] = "GAME";
    VFLColor[VFLColor["PLAYER"] = 10040012] = "PLAYER";
    VFLColor[VFLColor["TEAM"] = 16766720] = "TEAM";
})(VFLColor || (exports.VFLColor = VFLColor = {}));
class VFLEmbedBuilder {
    embed = {};
    constructor() {
     ...
     */
    static createGameBreakdownEmbed(game, stats) {
        const embed = new VFLEmbedBuilder()
        ...
        if (stats.keyPlayers && stats.keyPlayers.length > 0) {
            let keyPlayersText = '';
            stats.keyPlayers.forEach((player) => {
     ...
     */
    static createPlayerEmbed(player) {
        const embed = new VFLEmbedBuilder()
        ...
        if (player.weight)
            playerInfo += `**Weight:** ${player.weight} lbs\n`;
        if (player.college)
            playerInfo += `**College:** ${player.college}\n`;
        if (player.yearsPro !== undefined)
            playerInfo += `**Years Pro:** ${player.yearsPro}\n`;
        ...
        if (player.salary || player.contractYears) {
            let contractInfo = '';
            if (player.salary)
                contractInfo += `**Salary:** $${player.salary.toLocaleString()}\n`;
            if (player.contractYears)
                contractInfo += `**Contract Length:** ${player.contractYears} years\n`;
            if (player.contractStatus)
                contractInfo += `**Status:** ${player.contractStatus}\n`;
    ...
    static createTeamEmbed(team) {
        ...
        if (team.stats) {
            ...
            if (team.stats.pointsFor)
                offenseStats += `**Points For:** ${team.stats.pointsFor}\n`;
            if (team.stats.totalYards)
                offenseStats += `**Total Yards:** ${team.stats.totalYards}\n`;
            if (team.stats.passingYards)
                offenseStats += `**Passing:** ${team.stats.passingYards}\n`;
            if (team.stats.rushingYards)
                offenseStats += `**Rushing:** ${team.stats.rushingYards}\n`;
            let defenseStats = '';
            if (team.stats.pointsAgainst)
                defenseStats += `**Points Against:** ${team.stats.pointsAgainst}\n`;
            if (team.stats.yardsAllowed)
                defenseStats += `**Yards Allowed:** ${team.stats.yardsAllowed}\n`;
            if (team.stats.sacks)
                defenseStats += `**Sacks:** ${team.stats.sacks}\n`;
            if (team.stats.interceptions)
                defenseStats += `**Interceptions:** ${team.stats.interceptions}\n`;
            if (offenseStats)
                embed.addField('⚡ Offense', offenseStats, true);
            if (defenseStats)
                embed.addField('🛡️ Defense', defenseStats, true);
        }
     ...
     */
    static createStandingsEmbed(standings, conference) {
        const embed = new VFLEmbedBuilder()
...
exports.VFLEmbedBuilder = VFLEmbedBuilder;
