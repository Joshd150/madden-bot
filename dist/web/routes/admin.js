 ...
 */
const router = new router_1.default({ prefix: "/admin" });
...
router.post("/login", async (ctx) => {
    ...
    if (email === ADMIN_EMAIL && password === ADMIN_PASSWORD) {
        const token = jsonwebtoken_1.default.sign({ email, role: 'admin', loginTime: Date.now() }, JWT_SECRET, { expiresIn: '24h' });
        // Set secure cookie
...
router.get("/api/overview", requireAuth, async (ctx) => {
    try {
        ...
        const [teamsSnapshot, playersSnapshot, tradesSnapshot, gamesSnapshot, configSnapshot] = await Promise.all([
            firebase_1.default.collection('vfl_teams').get(),
            firebase_1.default.collection('vfl_players').get(),
            firebase_1.default.collection('vfl_trades').get(),
            firebase_1.default.collection('vfl_games').get(),
            firebase_1.default.collection('vfl_config').get()
        ]);
        // Get recent activity
        const recentTradesSnapshot = await firebase_1.default.collection('vfl_trades')
...
router.get("/api/bot-configs", requireAuth, async (ctx) => {
    try {
        const configsSnapshot = await firebase_1.default.collection('vfl_config').get();
...
router.put("/api/bot-configs/:guildId", requireAuth, async (ctx) => {
    ...
    try {
        await firebase_1.default.collection('vfl_config').doc(guildId).set(config, { merge: true });
...
router.post("/api/teams", requireAuth, async (ctx) => {
    ...
    try {
        const docRef = await firebase_1.default.collection('vfl_teams').add({
            ...teamData,
            createdAt: new Date(),
            updatedAt: new Date()
        });
...
router.put("/api/teams/:teamId", requireAuth, async (ctx) => {
    ...
    const teamData = ctx.request.body;
    try {
        await firebase_1.default.collection('vfl_teams').doc(teamId).update({
            ...teamData,
            updatedAt: new Date()
        });
...
router.delete("/api/teams/:teamId", requireAuth, async (ctx) => {
    const { teamId } = ctx.params;
    try {
        await firebase_1.default.collection('vfl_teams').doc(teamId).delete();
...
router.post("/api/trades", requireAuth, async (ctx) => {
    ...
    try {
        const docRef = await firebase_1.default.collection('vfl_trades').add({
            ...tradeData,
            tradeDate: new Date(),
...
router.get("/api/system-status", requireAuth, async (ctx) => {
    try {
        ...
        const testQuery = await firebase_1.default.collection('vfl_config').limit(1).get();
        const status = {
            database: 'healthy',
            api: 'healthy',
