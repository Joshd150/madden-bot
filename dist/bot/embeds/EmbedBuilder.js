...
exports.CustomEmbedBuilder = void 0;
...
class CustomEmbedBuilder {
    ...
    constructor() {
        ...
        this.embed.setColor(config_1.config.bot.defaultEmbedColor);
    ...
    static createPlayerEmbed(player) {
        ...
        if (player.salary || player.contractYears) {
            ...
            if (player.contractYears)
                contractInfo += `**Contract:** ${player.contractYears} years\n`;
            embed.addFields({
    ...
    static createTeamEmbed(team) {
        ...
        if (team.stats) {
            ...
            if (team.stats.pointsFor)
                statsText += `**Points For:** ${team.stats.pointsFor}\n`;
            if (team.stats.pointsAgainst)
                statsText += `**Points Against:** ${team.stats.pointsAgainst}\n`;
            if (team.stats.totalYards)
                statsText += `**Total Yards:** ${team.stats.totalYards}\n`;
            if (team.stats.yardsAllowed)
                statsText += `**Yards Allowed:** ${team.stats.yardsAllowed}\n`;
            embed.addFields({
                name: '📊 Team Stats',
                value: statsText,
                inline: true
        ...
        if (team.recentGames && team.recentGames.length > 0) {
            let recentGamesText = '';
            team.recentGames.slice(0, 3).forEach((game) => {
     ...
     */
    static createErrorEmbed(message, details) {
        const embed = new discord_js_1.EmbedBuilder()
     ...
     */
    static createSuccessEmbed(message, details) {
        const embed = new discord_js_1.EmbedBuilder()
