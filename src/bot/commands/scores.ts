...
import { CustomEmbedBuilder } from '../embeds/EmbedBuilder';
...
export const scoresCommand = {
  ...
  async execute(interaction: ChatInputCommandInteraction) {
    ...
    try {
      switch (subcommand) {
        ...
        default:
          await interaction.reply({
            embeds: [CustomEmbedBuilder.createErrorEmbed('Unknown subcommand')],
            ephemeral: true
          });
      }
    } catch (error) {
      ...
      await interaction.reply({
        embeds: [CustomEmbedBuilder.createErrorEmbed(
  ...
  async autocomplete(interaction: any) {
    ...
    if (focusedOption.name === 'team' || focusedOption.name === 'home_team' || focusedOption.name === 'away_team') {
      const teams = await database.all(
        'SELECT name, abbreviation FROM teams WHERE name LIKE ? OR abbreviation LIKE ? LIMIT 25',
        [`%${focusedOption.value}%`, `%${focusedOption.value}%`]
      );
 ...
 */
async function handleRecentScores(interaction: ChatInputCommandInteraction) {
  ...
  if (games.length === 0) {
    await interaction.reply({
      embeds: [CustomEmbedBuilder.createInfoEmbed(
        'No Recent Games',
        'There are no recent completed games to display.'
      )],
      ephemeral: true
    });
    return;
  ...
  for (const game of games) {
    ...
    const embed = CustomEmbedBuilder.createGameEmbed(gameData);
...
async function handleSchedule(interaction: ChatInputCommandInteraction) {
  ...
  if (games.length === 0) {
    await interaction.reply({
      embeds: [CustomEmbedBuilder.createInfoEmbed(
        'No Scheduled Games',
        week ? `No games scheduled for week ${week}.` : 'No upcoming games scheduled.'
      )],
      ephemeral: true
    });
    return;
  ...
  const gamesByWeek = games.reduce((acc: any, game) => {
    if (!acc[game.week]) {
      acc[game.week] = [];
    }
  ...
  for (const [weekNum, weekGames] of Object.entries(gamesByWeek)) {
    const embed = CustomEmbedBuilder.createInfoEmbed(
      `Week ${weekNum} Schedule`,
      `${(weekGames as any[]).length} games scheduled`
    );

    let scheduleText = '';
    (weekGames as any[]).forEach(game => {
 ...
 */
async function handleTeamGames(interaction: ChatInputCommandInteraction) {
  ...
  if (games.length === 0) {
    await interaction.reply({
      embeds: [CustomEmbedBuilder.createInfoEmbed(
        'No Games Found',
        `${team.name} has no ${showUpcoming ? 'upcoming' : 'recent'} games.`
      )],
      ephemeral: true
    });
    return;
  ...
  const embed = CustomEmbedBuilder.createInfoEmbed(
    `${team.name} ${showUpcoming ? 'Schedule' : 'Recent Games'}`,
    `${games.length} ${showUpcoming ? 'upcoming' : 'completed'} games`
  );
 ...
 */
async function handleGameBreakdown(interaction: ChatInputCommandInteraction) {
  ...
  const embed = CustomEmbedBuilder.createGameBreakdownEmbed(gameData, stats);
 ...
 */
async function handleAddGame(interaction: ChatInputCommandInteraction) {
  ...
  // Find teams
  const homeTeam = await database.get(
    'SELECT * FROM teams WHERE name LIKE ? OR abbreviation LIKE ?',
    [`%${homeTeamName}%`, `%${homeTeamName}%`]
  );

  const awayTeam = await database.get(
    'SELECT * FROM teams WHERE name LIKE ? OR abbreviation LIKE ?',
  ...
  await interaction.reply({
    embeds: [CustomEmbedBuilder.createSuccessEmbed(
      'Game Result Added',
      `${awayTeam.name} ${awayScore} - ${homeScore} ${homeTeam.name} (Week ${week})`
    )]
  });
