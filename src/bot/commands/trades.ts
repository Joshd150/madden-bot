  ...
  async autocomplete(interaction: any) {
    ...
    if (focusedOption.name === 'team') {
      ...
      const players = await database.all(
        'SELECT name, position FROM players WHERE name LIKE ? LIMIT 25',
        [`%${focusedOption.value}%`]
      );
  ...
  if (trades.length === 0) {
    await interaction.reply({
      embeds: [CustomEmbedBuilder.createInfoEmbed(
        'No Trades Found',
        'There are no recent trades to display.'
      )],
      ephemeral: true
    });
    return;
  ...
  for (const trade of trades.slice(0, 10)) {
    ...
    for (const playerId of playerIds) {
      const player = await database.get(
        'SELECT name, position FROM players WHERE id = ?',
        [playerId]
      );
      if (player) {
        players.push({
    ...
    const embed = CustomEmbedBuilder.createTradeEmbed(tradeData);
    embed.addFields({
      name: '📅 Trade Date',
 ...
 */
async function handleTeamTrades(interaction: ChatInputCommandInteraction) {
  ...
  if (trades.length === 0) {
    await interaction.reply({
      embeds: [CustomEmbedBuilder.createInfoEmbed(
        'No Trades Found',
        `${team.name} has no recent trades.`
      )],
      ephemeral: true
    });
    return;
 ...
 */
async function handlePlayerTrades(interaction: ChatInputCommandInteraction) {
  ...
  if (!player) {
    await interaction.reply({
      embeds: [CustomEmbedBuilder.createErrorEmbed(
        'Player Not Found',
        `Could not find a player matching "${playerName}"`
      )],
      ephemeral: true
    });
    return;
  ...
  if (trades.length === 0) {
    await interaction.reply({
      embeds: [CustomEmbedBuilder.createInfoEmbed(
        'No Trades Found',
        `${player.name} has not been involved in any recent trades.`
      )],
      ephemeral: true
    });
    return;
 ...
 */
async function handleAddTrade(interaction: ChatInputCommandInteraction) {
  ...
  // Find teams
  const fromTeam = await database.get(
    'SELECT * FROM teams WHERE name LIKE ? OR abbreviation LIKE ?',
    [`%${fromTeamName}%`, `%${fromTeamName}%`]
  );

  const toTeam = await database.get(
    'SELECT * FROM teams WHERE name LIKE ? OR abbreviation LIKE ?',
  ...
  if (playerIds.length === 0) {
    await interaction.reply({
      embeds: [CustomEmbedBuilder.createErrorEmbed(
        'Players Not Found',
        'Could not find any of the specified players.'
      )],
      ephemeral: true
    });
    return;
  ...
  await interaction.reply({
    embeds: [CustomEmbedBuilder.createSuccessEmbed(
      ...
      `Trade between ${fromTeam.name} and ${toTeam.name} has been recorded.`
    )]
  });
