// REMOVE these! You should NEVER hardcode .env values in code.
...
// Optionally load dotenv here if you haven't already at app entrypoint
// import * as dotenv from "dotenv";
// dotenv.config();

const router = new Router({ prefix: "/discord/webhook" });

// Log envs for debugging. (Remove these lines after confirming env works!)
console.log("PUBLIC_KEY:", process.env.PUBLIC_KEY ? process.env.PUBLIC_KEY.slice(0, 8) + "..." : "Not set");
console.log("DISCORD_TOKEN:", process.env.DISCORD_TOKEN ? process.env.DISCORD_TOKEN.slice(0, 8) + "..." : "Not set");
console.log("APP_ID:", process.env.APP_ID ? process.env.APP_ID : "Not set");
  ...
  events.map(async (broadcastEvent) => {
    ...
    if (!configuration) {
      ...
      try {
        ...
        console.error("could not send broadcast");
      }
    }
  });
});
...
async function updateScoreboard(leagueSettings: LeagueSettings, guildId: string, seasonIndex: number, week: number) {
  ...
  try {
    ...
    const sims = await EventDB.queryEvents<ConfirmedSim>(
      guildId,
      "CONFIRMED_SIM",
      new Date(0),
      { week: week, seasonIndex: seasonIndex },
      30
    );
    ...
    await Promise.all(
      querySnapshot.docs.map(async (leagueSettingsDoc) => {
        ...
        if (finishedGame) {
          ...
          await Promise.all(
            Object.values(settings.commands.game_channel?.weekly_states?.[createWeekKey(season, week)]?.channel_states || {}).map(
              async (channelState) => {
...
discordClient.on("guildMemberRemove", async (user, guild) => {
  ...
  if (leagueSettings.commands.teams) {
    ...
    await Promise.all(
      Object.entries(assignments).map(async (entry) => {
...
discordClient.on("messageReactionAdd", async (msg, reactor, reaction) => {
  ...
  await Promise.all(
    Object.values(weeklyStates).map(async (weeklyState: any) => {
      await Promise.all(
        Object.entries(weeklyState.channel_states).map(async ([channelId, channelState]: any) => {
